...
info "Installing Yollama binary to $INSTALL_BIN_DIR..."
install -m 755 "$BINARY_NAME" "$INSTALL_BIN_DIR/$BINARY_NAME"
success "Binary installed successfully."

info "Creating shared library directory at $INSTALL_LIB_DIR..."
mkdir -p "$INSTALL_LIB_DIR"

# If you have a local 'lib' folder containing the .so files during manual install:
if [ -d "lib" ]; then
  info "Copying llama.cpp / ggml shared runtimes and assets to $INSTALL_LIB_DIR..."
  cp -P lib/*.so* "$INSTALL_LIB_DIR/" 2>/dev/null || true
  cp lib/llama-server "$INSTALL_LIB_DIR/" 2>/dev/null || true
  cp lib/llama-quantize "$INSTALL_LIB_DIR/" 2>/dev/null || true
  success "Shared libraries and components mapped successfully."
else
