...
static void * yollama_dlopen(const char * path, int global) {
	return dlopen(path, RTLD_NOW | (global ? RTLD_GLOBAL : RTLD_LOCAL));
}

static void * yollama_dlsym(void * handle, const char * name) {
	return dlsym(handle, name);
}

static const char * yollama_dlerror(void) {
	const char * err = dlerror();
	return err ? err : "";
}

typedef void * (*yollama_ggml_backend_load_fn)(const char *);
typedef size_t (*yollama_ggml_backend_reg_dev_count_fn)(void *);
typedef void * (*yollama_ggml_backend_reg_dev_get_fn)(void *, size_t);
typedef const char * (*yollama_ggml_backend_reg_name_fn)(void *);
typedef void (*yollama_ggml_backend_dev_get_props_fn)(void *, void *);

static void * yollama_call_ggml_backend_load(void * fn, const char * path) {
	return ((yollama_ggml_backend_load_fn) fn)(path);
}

static size_t yollama_call_ggml_backend_reg_dev_count(void * fn, void * reg) {
	return ((yollama_ggml_backend_reg_dev_count_fn) fn)(reg);
}

static void * yollama_call_ggml_backend_reg_dev_get(void * fn, void * reg, size_t index) {
	return ((yollama_ggml_backend_reg_dev_get_fn) fn)(reg, index);
}

static const char * yollama_call_ggml_backend_reg_name(void * fn, void * reg) {
	return ((yollama_ggml_backend_reg_name_fn) fn)(reg);
}

static void yollama_call_ggml_backend_dev_get_props(void * fn, void * dev, void * props) {
	((yollama_ggml_backend_dev_get_props_fn) fn)(dev, props);
}

static const char * yollama_cstr_from_uintptr(uintptr_t ptr) {
	return (const char *) ptr;
}

typedef int (*yollama_cu_init_fn)(unsigned int);
typedef int (*yollama_cu_driver_get_version_fn)(int *);
typedef int (*yollama_cu_device_get_count_fn)(int *);
typedef int (*yollama_cu_device_get_fn)(int *, int);
typedef int (*yollama_cu_device_get_attribute_fn)(int *, int, int);
typedef int (*yollama_cu_device_get_name_fn)(char *, int, int);
typedef int (*yollama_cu_device_total_mem_fn)(size_t *, int);
typedef int (*yollama_cu_device_get_pci_bus_id_fn)(char *, int, int);

static int yollama_call_cu_init(void * fn) {
	return ((yollama_cu_init_fn) fn)(0);
}

static int yollama_call_cu_driver_get_version(void * fn, int * version) {
	return ((yollama_cu_driver_get_version_fn) fn)(version);
}

static int yollama_call_cu_device_get_count(void * fn, int * count) {
	return ((yollama_cu_device_get_count_fn) fn)(count);
}

static int yollama_call_cu_device_get(void * fn, int * device, int index) {
	return ((yollama_cu_device_get_fn) fn)(device, index);
}

static int yollama_call_cu_device_get_attribute(void * fn, int * value, int attr, int device) {
	return ((yollama_cu_device_get_attribute_fn) fn)(value, attr, device);
}

static int yollama_call_cu_device_get_name(void * fn, char * name, int len, int device) {
	return ((yollama_cu_device_get_name_fn) fn)(name, len, device);
}

static int yollama_call_cu_device_total_mem(void * fn, size_t * total, int device) {
	return ((yollama_cu_device_total_mem_fn) fn)(total, device);
}

static int yollama_call_cu_device_get_pci_bus_id(void * fn, char * pci, int len, int device) {
	return ((yollama_cu_device_get_pci_bus_id_fn) fn)(pci, len, device);
}

typedef int (*yollama_nvml_init_fn)(void);
typedef int (*yollama_nvml_shutdown_fn)(void);
typedef int (*yollama_nvml_system_get_driver_version_fn)(char *, unsigned int);

static int yollama_call_nvml_init(void * fn) {
	return ((yollama_nvml_init_fn) fn)();
}

static int yollama_call_nvml_shutdown(void * fn) {
	return ((yollama_nvml_shutdown_fn) fn)();
}

static int yollama_call_nvml_system_get_driver_version(void * fn, char * version, unsigned int len) {
	return ((yollama_nvml_system_get_driver_version_fn) fn)(version, len);
}
...
func probeCUDADriverLinux() ([]nativeProbeDevice, error) {
	...
	if ret := C.yollama_call_cu_init(cuInit); ret != cuSuccess {
		return nil, fmt.Errorf("cuInit failed: %d", int(ret))
	}

	var driverVersion C.int
	driverMajor, driverMinor := 0, 0
	if ret := C.yollama_call_cu_driver_get_version(cuDriverGetVersion, &driverVersion); ret == cuSuccess {
	...
	var count C.int
	if ret := C.yollama_call_cu_device_get_count(cuDeviceGetCount, &count); ret != cuSuccess {
	...
	for i := range deviceCount {
		var device C.int
		if ret := C.yollama_call_cu_device_get(cuDeviceGet, &device, C.int(i)); ret != cuSuccess {
			continue
		}
		...
		var name [128]C.char
		_ = C.yollama_call_cu_device_get_name(cuDeviceGetName, &name[0], C.int(len(name)), device)

		var total C.size_t
		_ = C.yollama_call_cu_device_total_mem(cuDeviceTotalMem, &total, device)

		pci := ""
		if cuDeviceGetPCIBusID != nil {
			var pciBuf [32]C.char
			if ret := C.yollama_call_cu_device_get_pci_bus_id(cuDeviceGetPCIBusID, &pciBuf[0], C.int(len(pciBuf)), device); ret == cuSuccess {
...
func probeNVIDIADriverMajorLinux() (int, error) {
	...
	if ret := C.yollama_call_nvml_init(initFn); ret != 0 {
		return 0, fmt.Errorf("nvmlInit_v2 failed: %d", int(ret))
	}
	defer C.yollama_call_nvml_shutdown(shutdownFn)

	var version [80]C.char
	if ret := C.yollama_call_nvml_system_get_driver_version(driverFn, &version[0], C.uint(len(version))); ret != 0 {
...
func cudaDeviceAttribute(fn unsafe.Pointer, attr int, device C.int) int {
	var value C.int
	if ret := C.yollama_call_cu_device_get_attribute(fn, &value, C.int(attr), device); ret != cuSuccess {
		return 0
	}
	return int(value)
}
...
func dlopen(path string, global bool) (dlHandle, error) {
	...
	handle := C.yollama_dlopen(cpath, boolToCInt(global))
	if handle == nil {
		return dlHandle{}, fmt.Errorf("dlopen %s: %s", path, C.GoString(C.yollama_dlerror()))
	}
...
func dlsym(handle dlHandle, name string) (unsafe.Pointer, error) {
	...
	sym := C.yollama_dlsym(handle.ptr, cname)
	if sym == nil {
		return nil, fmt.Errorf("dlsym %s: %s", name, C.GoString(C.yollama_dlerror()))
	}
	return sym, nil
}
...
func cString(ptr uintptr) string {
	...
	return C.GoString(C.yollama_cstr_from_uintptr(C.uintptr_t(ptr)))
}
